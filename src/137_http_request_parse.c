...
//the main status machine has two status, parsing line and parsing head.
enum CHECK_STATE{CHECK_STATE_REQUESTLINE = 0, CHECK_STATE_HEADER};
//the slaver status machine has three status, complete line, bad line, no-complete line.
enum LINE_STATUS{LINE_OK = 0, LINE_BAD, LINE_OPEN};
//the status machine of http requirement.
...
enum HTTP_CODE prase_headers(char* temp) 
{
	if (temp[0] == '\0') {
		return GET_REQUEST;
